...
class FreshKartPandasPipeline:
    ...
    def filter_paid_orders(self) -> pd.DataFrame:
        ...
        """
        paid_orders = self.orders_df[
            self.orders_df['payment_status'] == 'paid'
        ].copy()
    ...
    def explode_items(self, orders_df: pd.DataFrame) -> pd.DataFrame:
        ...
        result = pd.concat([
            exploded.drop(columns=['items']).reset_index(drop=True),
            items_df.add_prefix('item_')
        ], axis=1)
    ...
    def calculate_revenue(self, orders_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        orders_df['line_revenue'] = (
            orders_df['item_qty'] * orders_df['item_unit_price']
        )
        
        # Agréger par commande
        revenue_df = orders_df.groupby([
    ...
    def join_with_customers(self, orders_df: pd.DataFrame) -> pd.DataFrame:
        ...
        result = orders_df.merge(
            self.customers_df[['customer_id', 'city', 'is_active']],
            on='customer_id',
            how='left'
        )
    ...
    def aggregate_daily_stats(self, orders_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Agréger
        stats = orders_df.groupby(['date', 'city', 'channel']).agg({
            'order_id': 'nunique',
            'customer_id': 'nunique',
            'total_items': 'sum',
            'gross_revenue': 'sum',
