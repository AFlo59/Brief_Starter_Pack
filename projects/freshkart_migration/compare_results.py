...
def compare_pipelines(data_path):
    ...
    print("-" * 75)
    print(f"{'Nb Clients':<30} {len(customers_pd):<15} {customers_spark.count():<15} {len(customers_pd) - customers_spark.count():<15}")
    print(f"{'Nb Commandes':<30} {len(orders_pd):<15} {orders_spark.count():<15} {len(orders_pd) - orders_spark.count():<15}")
    print(f"{'Nb Remboursements':<30} {len(refunds_pd):<15} {refunds_spark.count():<15} {len(refunds_pd) - refunds_spark.count():<15}")
    print(f"{'Temps chargement (s)':<30} {time_pandas_load:<15.2f} {time_pyspark_load:<15.2f} {time_pandas_load - time_pyspark_load:<15.2f}")
    ...
    print(f"\n{'Métrique':<30} {'Pandas':<15} {'PySpark':<15} {'Diff':<15}")
    print("-" * 75)
    print(f"{'Nb lignes finales':<30} {len(result_pd):<15} {result_spark.count():<15} {len(result_pd) - result_spark.count():<15}")
    print(f"{'Nb colonnes':<30} {len(result_pd.columns):<15} {len(result_spark.columns):<15} {len(result_pd.columns) - len(result_spark.columns):<15}")
    print(f"{'Temps exécution (s)':<30} {time_pandas_full:<15.2f} {time_pyspark_full:<15.2f} {time_pandas_full - time_pyspark_full:<15.2f}")
    ...
    print("-" * 90)
    print(f"{'Revenu total (€)':<30} {total_revenue_pd:<20.2f} {total_revenue_spark:<20.2f} {abs(total_revenue_pd - total_revenue_spark):<20.2f}")
    print(f"{'Revenu moyen (€)':<30} {mean_revenue_pd:<20.2f} {mean_revenue_spark:<20.2f} {abs(mean_revenue_pd - mean_revenue_spark):<20.2f}")
    ...
    print(f"⚠️  Colonnes uniquement Pandas : {cols_pd - cols_spark if cols_pd - cols_spark else 'Aucune'}")
    print(f"⚠️  Colonnes uniquement PySpark : {cols_spark - cols_pd if cols_spark - cols_pd else 'Aucune'}")
    ...
    print("\n🧪 Tests de validation :")
    print(f"  {'Nb lignes identiques':<40} {'✅ PASS' if diff_rows == 0 else f'❌ FAIL (diff: {diff_rows})'}")
    print(f"  {'Colonnes identiques':<40} {'✅ PASS' if cols_pd == cols_spark else '❌ FAIL'}")
    print(f"  {'Revenu total proche (±1€)':<40} {'✅ PASS' if diff_revenue < 1.0 else f'❌ FAIL (diff: {diff_revenue:.2f}€)'}")
    print(f"  {'Temps exécution':<40} {'⚡ PySpark' if time_pyspark_full < time_pandas_full else '🐢 Pandas'} plus rapide")
