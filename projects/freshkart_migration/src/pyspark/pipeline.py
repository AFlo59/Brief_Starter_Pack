...
class FreshKartPySparkPipeline:
    ...
    def load_data(self):
        ...
        self.refunds_df = self.spark.read \
            .option("header", "true") \
            .option("inferSchema", "true") \
            .csv(refunds_path)
    ...
    def calculate_revenue(self, orders_df):
        ...
        revenue_df = with_revenue.groupBy(
            "order_id", "customer_id", "order_date", "channel"
        ).agg(
            sum("item_qty").alias("total_items"),
            sum("line_revenue").alias("gross_revenue")
        )
    ...
    def add_refunds(self, orders_df):
        ...
        refunds_agg = self.refunds_df.groupBy("order_id") \
            .agg(sum("amount").alias("refund_amount"))
        ...
        result = result.fillna({"refund_amount": 0.0}) \
            .withColumn("net_revenue", col("gross_revenue") + col("refund_amount"))
    ...
    def aggregate_daily_stats(self, orders_df):
        ...
        # Agréger
        stats = with_date.groupBy("date", "city", "channel").agg(
